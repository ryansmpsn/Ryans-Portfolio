    ...
    render() {
      for (let i = 0; i <= this.movingTotal - 1; ++i) {
        ...
        this.translations[i].x = lerp(
          ...
          lineEq(this.options.valuesFromTo[1], this.options.valuesFromTo[0], winsize.width, 0, this.mousePos.x),
          lerpFactor
        );
        this.translations[i].y = lerp(
          this.translations[i].y,
          lineEq(this.options.valuesFromTo[1], this.options.valuesFromTo[0], winsize.height, 0, this.mousePos.y),
          lerpFactor
        );
  ...
  class Slideshow {
    ...
    toggleContent(action) {
      ...
      times.textsExtraTitles =
        action === "show" ? times.texts : Number(0.05 + 0.04 * (currentSlide.titleLettersTotal - 1) + times.texts);
      ...
      times.contentExtraTitles =
        action === "show" ? Number(0.05 + 0.04 * (currentSlide.titleLettersTotal - 1) + times.content) : times.content;
    ...
    toggleSlides(currentSlide, upcomingSlide) {
      ...
      this.tl.addCallback(
        ...
        this.animationSettings.duration + this.animationSettings.staggerFactor * (this.slidesTotal - 1)
      );
